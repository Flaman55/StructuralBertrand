...
\newcommand{\Formal}[1]{\par\smallskip\noindent\emph{Formalization.} #1\par\smallskip}

\title{A Structural Sieve for Bertrand's Postulate:\\
The Deterministic Reach of the Prime Base and its Newton Closure}
...
\url{https://github.com/Flaman55/StructuralBertrand}.}
...
A structural preliminary shows the reduction is not vacuous: the largest ring
$P_{\max}$ exerts no force inside its own window (its first multiple $2P_{\max}$ is the
lone edge point), and the previous base carries a weight $w=P_{\max}\cdot\varphi(M)/M\ge1$.
The weight being $\ge1$ is the necessary condition for a void; the atom is the sufficiency.
...
\lean{anchor\_idle\_in\_own\_window}.}
...
For $n\ge512$ the two bounds are combined. Suppose $(n,2n]$ has no prime.
Theorem~\ref{thm:upper} bounds $\binom{2n}{n}$ by its old-source content, at most
$(2n)^{\sqrt{2n}}\,4^{2n/3}$; combined with the lower bound and the size threshold one
obtains $4^n<4^n$, a contradiction.
